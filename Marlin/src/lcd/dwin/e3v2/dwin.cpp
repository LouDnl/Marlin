...
#include "../../../gcode/gcode.h" // Smith3D.com addition
...
select_t select_page{0}, select_file{0}, select_print{0}, select_prepare{0}
         , select_control{0}, select_axis{0}, select_aux{0}, select_ztool{0}, select_refuel{0}, select_temp{0}, select_motion{0}, select_tune{0} // Smith3D.com addition
...
uint8_t index_file     = MROWS,
        index_prepare  = MROWS,
        index_aux = MROWS, // Smith3D.com addition					  
        index_control  = MROWS,
...
#endif
constexpr float default_axis_steps_per_unit[] = DEFAULT_AXIS_STEPS_PER_UNIT; // Smith3D.com addition
...
void HMI_SetLanguage() {
  ...
  #endif
  HMI_flag.language = DWIN_ENGLISH; //Force English // Smith3D.com addition												   
...
#define PREPARE_CASE_ZTOOL (PREPARE_CASE_ZOFF + ENABLED(HAS_ZOFFSET_ITEM)) // Smith3D.com addition
#define PREPARE_CASE_AUX (PREPARE_CASE_ZTOOL + 1) // Smith3D.com addition
#define PREPARE_CASE_REFUEL (PREPARE_CASE_AUX + 1) // Smith3D.com addition
#define PREPARE_CASE_PLA  (PREPARE_CASE_REFUEL + ENABLED(HAS_HOTEND)) // Smith3D.com addition
...
inline void Draw_Prepare_Menu() {
  ...
  if (PVISI(PREPARE_CASE_HOME)) Item_Prepare_Home(PSCROL(PREPARE_CASE_HOME));     // Auto Home
    ...
    if (PVISI(PREPARE_CASE_ZTOOL)) Item_Prepare_ZTool(PSCROL(PREPARE_CASE_ZTOOL)); // Z Tool > // Smith3D.com addition
  #endif
    if (PVISI(PREPARE_CASE_AUX)) Item_Prepare_AUX(PSCROL(PREPARE_CASE_AUX));         // AUX Leveling > // Smith3D.com addition
    if (PVISI(PREPARE_CASE_REFUEL)) Item_Prepare_Refuel(PSCROL(PREPARE_CASE_REFUEL));  // Filament Feed > // Smith3D.com addition
  #if HAS_HOTEND
...
//
  ...
  void HMI_Zoffset() {
    ENCODER_DiffState encoder_diffState = Encoder_ReceiveAnalyze();
    if (encoder_diffState != ENCODER_DIFF_NO) {
	  last_zoffset = dwin_zoffset; // Smith3D.com addition							  
      uint8_t zoff_line;
...
void HMI_MainMenu() {
  ...
  if (encoder_diffState == ENCODER_DIFF_CW) {
    ...
    switch (select_page.now) {
      case 0: // Print File
        checkkey = SelectFile;
		gcode.process_subcommands_now_P(PSTR("M21" )); //Support MicroSD Card Extension	// Smith3D.com addition
...
inline void Draw_ZTool_Menu() {
  Clear_Main_Window();
  Draw_Title("Z Tool"); // by [Smith3D.com] TODO: GET_TEXT_F
...
inline void Draw_Refuel_Menu() {
  Clear_Main_Window();
  Draw_Title("Refuel Filament"); // by [Smith3D.com] TODO: GET_TEXT_F
...
inline void Draw_AUX_Menu() {
 ...
 if (HMI_IsChinese()) {
   ...
   #ifdef USE_STRING_HEADINGS
      Draw_Title("AUX Leveling"); // by [Smith3D.com] TODO: GET_TEXT_F
    #else
  ...
  if (encoder_diffState == ENCODER_DIFF_CW) {
    if (select_prepare.inc(1 + PREPARE_CASE_TOTAL)) {
      if (select_prepare.now > MROWS && select_prepare.now > index_prepare) {
        ...
        if (index_prepare < 7) Draw_More_Icon(MROWS - index_prepare + 1);
          if (index_prepare == PREPARE_CASE_AUX) Item_Prepare_AUX(MROWS); // Smith3D.com addition
          if (index_prepare == PREPARE_CASE_REFUEL) Item_Prepare_Refuel(MROWS); // Smith3D.com addition
        #if HAS_HOTEND
    ...
    if (select_prepare.dec()) {
      if (select_prepare.now < index_prepare - MROWS) {
        ...
        else if (index_prepare == 9) Item_Prepare_Offset(0); // Smith3D.com addition
        else if (index_prepare == 10) Item_Prepare_ZTool(0); // Smith3D.com addition
        else if (index_prepare == 11) Item_Prepare_Refuel(0); // Smith3D.com addition
      }
      else {
        Move_Highlight(-1, select_prepare.now + MROWS - index_prepare);
      }
    ...
    switch (select_prepare.now) {
      ...
      case PREPARE_CASE_HOME: // Homing
        ...
        index_prepare = MROWS;
		gcode.process_subcommands_now_P( PSTR("M220 S100")); // Smith3D.com addition												
